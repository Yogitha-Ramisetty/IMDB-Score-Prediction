...
```{r}
IMDB = read.csv("C:/Users/nanda/Downloads/movie_metadata.csv")
str(IMDB)
```
...
From the means of imdb score for different aspect ratios, we can see there is no significant difference, all the means fall in the range of 6.3~6.6. So, removing this variable won't affect our following analysis.
...
Now we finished imputing the numeric missing values. There are still some categorical missing values
...
* 1. If (user_vote >= 541000) then class = (8,10].

* 2. If (84000 <= user_vote < 541000) then class = (6,8].

* 3. If (user_vote < 84000) and (duration > 103) then class = (6,8].

* 4. If (user_vote < 84000) and (duration < 103) and (budget <=8300000) then class = (6,8].

* 5. If (user_vote < 84000) and (duration < 103) and (budget >=8300000) and (user_vote < 34000) then class = (4,6].

* 6. If (user_vote < 84000) and (duration < 103) and (budget >=8300000) and (user_vote > 34000) and (gross <= 17000000) then class = (6,8].

* 7. If (user_vote < 84000) and (duration < 103) and (budget >=8300000) and (user_vote > 34000) and (gross >= 17000000) and (critic_review_ratio < 1.5) then class = (4,6].

* 8. If (user_vote < 41000) and (duration < 103) and (budget >=8300000) and (user_vote > 34000) and (gross >= 17000000) and (critic_review_ratio > 1.5) then class = (6,8].
...
On the contrary, if a movie has fewer votes, it can still be a good movie if its duration is shorter (rule #3).
...
The 8th tree has the lowest cross-validation error (xerror): 0.7.
...
Accuracy is 0.753 for training set.
...
Accuracy is 0.724 for validation set.
...
Accuracy is 0.708 for test set.
...
When k = 9 and 13 we get the highest accuracy: 0.720
...
Dataset    | Decision Tree |  K-NN  |
-----------|---------------|--------|
Training   | 0.753         |        |          
Validation | 0.724         | 0.720  |     
Test       | 0.708         | 0.686  |    
...
Based on the overall performance, we find the best model is decision tree, which gives a high accuracy around 0.753.
